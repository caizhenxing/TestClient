         ...
         Ext.Ajax.request({
            ...
            success: function(response, opts) {
            	...
            	Ext.getCmp('InputPanel').add({
	        		...
	        		name:'folderName8582439514'
				});
            	
            	for(var f in theparameters){
            		if(theparameters[f].type=='textfield'){
	            		Ext.getCmp('InputPanel').add({
					    xtype:'textfield',
					    labelWidth: 200,
	        			fieldLabel: theparameters[f].text,
	        			value:theparameters[f].defaultValue,
	        			name:theparameters[f].name
					    });
            		}else if(theparameters[f].type=='combo'){
            			Ext.getCmp('InputPanel').add({
					    xtype:'combo',
	        			labelWidth: 200,
	        			fieldLabel:theparameters[f].text,
	        			store:Ext.decode(theparameters[f].extraInfo),
	        			value:theparameters[f].defaultValue,
	        			name:theparameters[f].name
					    });
            		}else if(theparameters[f].type=='service'){
            			fieldName=f;
            			Ext.getCmp('InputPanel').add(
        					new Ext.create('Ext.form.field.ComboBox', {
        						...
        						fieldLabel:theparameters[f].text,
        						name:theparameters[f].name,
        						value:theparameters[f].extraInfo,
        						triggerAction:'all',
        						selectedClass:'',
            		...
            		}else if(theparameters[f].type.indexOf('label')==0){
            			fieldName=f;
            			var len=theparameters[f].extraInfo.indexOf('+');
            			Ext.getCmp('InputPanel').add({
            				xtype:'label',
//            				border: 1,
//            				style: {
...
//            				    borderStyle: 'solid'
//            				},
            				html: '<p><font color="blue">'+theparameters[f].text+"：</font>"+theparameters[f].extraInfo.substr(0,len).replace('+','')
            			});
            		}else if(theparameters[f].type=='loop'){
            			Ext.getCmp('InputPanel').add({
					    xtype:'textfield',
					    labelWidth: 200,
					    fieldLabel: '循环次数',
            		...
            		}else if(theparameters[f].type=='hidden'){
            			Ext.getCmp('InputPanel').add({
					    xtype:'textfield',
	        			anchor: '100%',
	        			hidden:true,
